...
import com.fwtai.security.MyaccessDeniedException;
import com.fwtai.service.SysBackendApiTableService;
...
public class DynamicPermission {
    ...
    @Autowired
    private SysBackendApiTableService service;
    ...
    public boolean checkPermisstion(HttpServletRequest request,Authentication authentication) throws MyaccessDeniedException{
        ...
        if(principal instanceof UserDetails) {
            ...
            //通过账号获取资源鉴权
            final List<SysBackendApiTable> apiUrls = service.getApiUrlByUserName(username);
            ...
            //判断当前路径中是否在资源鉴权中
            final boolean rs = apiUrls.stream().anyMatch(item->{
                //判断URL是否匹配
                final boolean hashAntPath = antPathMatcher.match(item.getBackendApiUrl(),requestURI);
